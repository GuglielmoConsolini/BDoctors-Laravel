...
class DoctorSeeder extends Seeder
{
    public function run()
    {
        // Ottieni tutti gli utenti che sono medici (in questo esempio i primi 15)
        $users = DB::table('users')->take(15)->get(['id', 'name']); // Prendi i primi 15 utenti
        $userIds = $users->pluck('id');
        $userNames = $users->pluck('name', 'id');

        // Array con i dati dei dottori (assicurati che ci siano 15 elementi)
        $doctors = [
            [
                'surname' => 'Rossi',
                'address' => 'Via Roma, 1',
        ...
        // Inserisci i dottori associando l'user_id controllato
        foreach ($userIds as $index => $userId) {
            ...
            DB::table('doctors')->insert([
                ...
                'phone' => $doctor['phone'],
                'bio' => $doctor['bio'],
                'slug' => $userNames[$userId] . '-' . $doctor['surname'],
            ]);
        }
    }
}
